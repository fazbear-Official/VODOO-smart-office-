...
export default async function handler(req, res) {
  ...
  try {
    const { sessionId, collection, itemId, field, value } = req.body;
    ...
    existingData.items[itemIndex][field] = value;
    existingData.items[itemIndex].updatedAt = new Date().toISOString();
    existingData.metadata.lastUpdated = new Date().toISOString();
    
    await db.updateFile(filePath, JSON.stringify(existingData, null, 2), `Update field ${field} in ${collection}/${itemId}`);
